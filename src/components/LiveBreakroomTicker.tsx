...
export function LiveBreakroomTicker({ 
  ...
  useEffect(() => {
    if (messages.length > 0) {
      ...
      // Only show if it's not from the current user (optional, but usually better for "updates")
      // Actually, show all for "live feed" feel
      setLatestMessage(last);
      setShow(true);
      
      const timer = setTimeout(() => {
        // We could hide it after a while, or keep it as a sticky bottom bar
        // Let's keep it sticky but with an "unread" or "new" state
      }, 5000);
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 truncate">
            ...
            <span className="text-xs text-slate-300 truncate font-mono">
              {latestMessage.type === 'action' ? `*${latestMessage.content}*` : latestMessage.content}
            </span>
          </div>
        </div>
